...
function Header({ cols = [], title = "", board }) {
  ...
  if (width <= 576) {
    return (
      <div
        className={`h-24 px-4 py-5  border-b border-b-linesLight flex justify-between items-center
        `}
      >
        {!isActive ? (
          <div className=" transition-all flex">
            ...
            <span className="ml-8  block text-heading-xl mr-4">{title}</span>{" "}
            <Dialog.Root
            ...
            >
              ...
              <Dialog.Portal>
                <Dialog.Overlay className="bg-black/50 data-[state=open]:animate-overlayShow fixed right-0 left-0 bottom-0 top-[96px]" />
  ...
  return (
    <div
      className={`h-24 px-6  border-b border-b-linesLight flex justify-between items-center`}
    >
      {!isActive ? (
        <div className=" transition-all flex    h-[96px]">
          ...
          <span className="ml-8 py-8 block text-heading-xl">{title}</span>{" "}
        </div>
      ) : null}
      {isActive ? <span className="text-heading-xl">{title}</span> : null}{" "}
