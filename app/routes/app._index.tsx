...
export async function action({ request }: { request: Request }) {
  ...
  let boardDetails = Object.fromEntries(formData);
  const result = await getSupabase({ request })
    .from("board")
    .insert({ ...boardDetails, org_id: 21 });
  return result;
}
function AppIndex() {
  return (
    <>
      <Form
        method="post"
        className="max-w-md mx-auto p-4 bg-slate-50 border border-slate-200"
      >
